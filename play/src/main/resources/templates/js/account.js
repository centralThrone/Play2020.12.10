 ...
 */
	...
	function initCss() {
		...
		$('.pro:eq(6)').click(function() {
			$('.content').hide();
			login();
		});
		$('.pro:eq(7)').click(function() {
			$('.content').hide();
			register();
		});
	}
	...
	*/
	function buyTicket() {
		...
		function addUsers(data) {
			var str =
				'<button type="button" class="userbtn col-xs-12 list-group-item text-left" >\
                            <span>' +
				data.name + '</span>\
                            <span>' + data.tel +
				'</span>\
		...
		$('.buyTicket .buy01').append(
			'<button type="button" class ="nextbtn btn btn-success col-xs-offset-3 col-xs-6" style="height:40px;margin-top:20px;">下一步</button>'
		)
	...
	function userMessage() {
		...
		function setValue(user) {
			var str =
				'<button type="button" class="col-xs-12 list-group-item text-left" data-toggle="modal" data-target="#myModal">\
                <span>' +
				user.name + '</span>\
                <span>' + user.tel + '</span>\
                </button>'
	...
	function initAttention(data, flag, btnName) {
		...
		var str = '<div class="project col-xs-12">\
            ...
            <div class="l2 col-xs-2">\
                <p>热度：<span>' +
			data.hot + '</span> </p>' + pricestr +
			'\
            </div>\
            <div class="right col-xs-4">\
                <button class="see btn btn-success">' +
			btnName + '</button>\
            </div>\
        </div>'
		if (flag) {
	...
	// 在线服务
	function service() {
		var ans = ["有的，公园内有各式餐厅，为您提供包括糕点、面食、汉堡、传统中餐、西餐、饮料等等在内的中西美食，满足不同口味需求。", 
					"为了保证水质的安全，给大家提供一个干净舒适的游玩环境，同时也为了您的自身安全，园区禁止携带食品、水果、饮料、管制刀具、玻璃器皿等，如有携带，请您寄存在园外的免费寄存处。",
					 "园区游客服务中心可以受理租赁，婴儿车 使用费15元/天 ，押金200元；轮椅 使用费25元/天 ，押金300元。",
					  "门票当日有效，仅限使用一次，出园后再入园需重新购买。",
					   "身高1.1米以下的儿童（凭有效身份证件）注：免票人士须持有全价票票价成人陪同，每名持票成人限带一名免票人士",
					   "每周一到周五的早晨9点-晚7点，周末及节假日早晨九点-晚9点"]
		...
		for (var i = 0; i < pcolor.length; i++) {
			$(pcolor[i]).css('color', "black")
		}
		$('.service .ser_con .que p').click(function() {
			$(this).parent().hide()
			var value = $(this).html();
			var color = $(this).css("color");
			$('.service .ser_con .answer').show().find('p:eq(0)').css("color", color).html(value).end().find('p:eq(1)').html(
				ans[$(this).index()])
		})
